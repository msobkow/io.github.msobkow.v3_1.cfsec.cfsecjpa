// Description: Java 25 JPA implementation of SecRole history objects
...
/**
 *  CFSecJpaSecRoleH provides history objects matching the CFSecSecRole change history.
 */
@Entity
@Table(
    name = "SecRole_h", schema = "CFSec31",
    indexes = {
        @Index(name = "SecRoleIdIdx_h", columnList = "auditClusterId, auditStamp, auditAction, requiredRevision, auditSessionId, SecRoleId", unique = true),
        @Index(name = "SecRoleUNameIdx_h", columnList = "safe_name", unique = true)
    }
)
@Transactional(Transactional.TxType.SUPPORTS)
@PersistenceContext(unitName = "CFSecPU")
public class CFSecJpaSecRoleH
    implements ICFSecSecRoleH, Comparable<Object>, Serializable
{
	@AttributeOverrides({
    ...
    @EmbeddedId
    protected CFSecJpaSecRoleHPKey pkey;
	@AttributeOverrides({
    ...
    public CFSecJpaSecRoleH() {
            // The primary key member attributes are initialized on construction
            pkey = new CFSecJpaSecRoleHPKey();
    ...
    @Override
    public ICFSecSecRoleHPKey getPKey() {
        return( pkey );
    }

    @Override
    public void setPKey( ICFSecSecRoleHPKey pkey ) {
        if (pkey != null) {
            if (pkey instanceof CFSecJpaSecRoleHPKey) {
                this.pkey = (CFSecJpaSecRoleHPKey)pkey;
            }
            else {
                throw new CFLibUnsupportedClassException(getClass(), "setPKey", "pkey", pkey, "CFSecJpaSecRoleHPKey");
            }
        }
    }
    ...
    public String toString() {
        String ret = "<CFSecJpaSecRoleH" + getXmlAttrFragment() + "/>";
        return( ret );
    }
}
