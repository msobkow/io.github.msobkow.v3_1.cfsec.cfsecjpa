...
public class CFSecJpaSecClusGrpService {
	...
	public CFSecJpaSecClusGrp create(CFSecJpaSecClusGrp data) {
		...
		try {
			if(data.getPKey() != null && !data.getPKey().isNull() && cfsec31SecClusGrpRepository.existsById(($implCommaIJavaOptAtomType$)data.getPKey())) {
				return( (CFSecJpaSecClusGrp)(cfsec31SecClusGrpRepository.findById(($implCommaIJavaOptAtomType$)(data.getPKey())).get()));
			}
