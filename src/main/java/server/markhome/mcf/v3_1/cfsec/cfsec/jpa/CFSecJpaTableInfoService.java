...
public class CFSecJpaTableInfoService {
	...
	public CFSecJpaTableInfo create(CFSecJpaTableInfo data) {
		...
		try {
			if(data.getPKey() != null && cfsec31TableInfoRepository.existsById(($implCommaIJavaOptAtomType$)data.getPKey())) {
				return( (CFSecJpaTableInfo)(cfsec31TableInfoRepository.findById(($implCommaIJavaOptAtomType$)(data.getPKey())).get()));
			}
