...
public class CFSecJpaSecSysGrpService {
	...
	public CFSecJpaSecSysGrp create(CFSecJpaSecSysGrp data) {
		...
		try {
			if(data.getPKey() != null && !data.getPKey().isNull() && cfsec31SecSysGrpRepository.existsById(($implCommaIJavaOptAtomType$)data.getPKey())) {
				return( (CFSecJpaSecSysGrp)(cfsec31SecSysGrpRepository.findById(($implCommaIJavaOptAtomType$)(data.getPKey())).get()));
			}
