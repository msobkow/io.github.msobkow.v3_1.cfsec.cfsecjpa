...
public class CFSecJpaISOTZoneService {
	...
	public CFSecJpaISOTZone create(CFSecJpaISOTZone data) {
		...
		try {
			if(data.getPKey() != null && cfsec31ISOTZoneRepository.existsById(($implCommaIJavaOptAtomType$)data.getPKey())) {
				return( (CFSecJpaISOTZone)(cfsec31ISOTZoneRepository.findById(($implCommaIJavaOptAtomType$)(data.getPKey())).get()));
			}
