...
public class CFSecJpaISOCtryService {
	...
	public CFSecJpaISOCtry create(CFSecJpaISOCtry data) {
		...
		try {
			if(data.getPKey() != null && cfsec31ISOCtryRepository.existsById(($implCommaIJavaOptAtomType$)data.getPKey())) {
				return( (CFSecJpaISOCtry)(cfsec31ISOCtryRepository.findById(($implCommaIJavaOptAtomType$)(data.getPKey())).get()));
			}
