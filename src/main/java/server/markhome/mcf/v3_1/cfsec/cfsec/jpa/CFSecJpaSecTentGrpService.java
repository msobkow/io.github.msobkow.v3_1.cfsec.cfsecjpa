...
public class CFSecJpaSecTentGrpService {
	...
	public CFSecJpaSecTentGrp create(CFSecJpaSecTentGrp data) {
		...
		try {
			if(data.getPKey() != null && !data.getPKey().isNull() && cfsec31SecTentGrpRepository.existsById(($implCommaIJavaOptAtomType$)data.getPKey())) {
				return( (CFSecJpaSecTentGrp)(cfsec31SecTentGrpRepository.findById(($implCommaIJavaOptAtomType$)(data.getPKey())).get()));
			}
