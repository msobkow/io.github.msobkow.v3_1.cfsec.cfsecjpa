// Description: Java 25 JPA implementation of SecSysGrpMemb history objects
...
/**
 *  CFSecJpaSecSysGrpMembH provides history objects matching the CFSecSecSysGrpMemb change history.
...
@Entity
@Table(
    name = "SecSysGrpMemb_h", schema = "CFSec31",
    indexes = {
        @Index(name = "SecSysGrpMembIdIdx_h", columnList = "auditClusterId, auditStamp, auditAction, requiredRevision, auditSessionId, SecSysGrpId, login_id", unique = true),
        @Index(name = "SecSysGrpMembSysGrpIdx_h", columnList = "SecSysGrpId", unique = false),
        @Index(name = "SecSysGrpMembLoginIdx_h", columnList = "login_id", unique = false)
    }
)
@Transactional(Transactional.TxType.SUPPORTS)
@PersistenceContext(unitName = "CFSecPU")
public class CFSecJpaSecSysGrpMembH
    implements ICFSecSecSysGrpMembH, Comparable<Object>, Serializable
{
    ...
    @EmbeddedId
    protected CFSecJpaSecSysGrpMembHPKey pkey;
	@AttributeOverrides({
		@AttributeOverride( name="bytes", column = @Column( name="CreatedByUserId", nullable=false, length=CFLibDbKeyHash256.HASH_LENGTH ) )
	})
	protected CFLibDbKeyHash256 createdByUserId = CFLibDbKeyHash256.fromHex(ICFSec$emitScopingMidfix$SecSysGrpMemb.S_INIT_CREATED_BY);
	...
	protected CFLibDbKeyHash256 updatedByUserId = CFLibDbKeyHash256.fromHex(ICFSec$emitScopingMidfix$SecSysGrpMemb.S_INIT_UPDATED_BY);

	@Column(name="UpdatedAt", nullable=false)
	protected LocalDateTime updatedAt = LocalDateTime.now();

    public CFSecJpaSecSysGrpMembH() {
            // The primary key member attributes are initialized on construction
            pkey = new CFSecJpaSecSysGrpMembHPKey();
    }
    ...
    public void setPKey( ICFSecSecSysGrpMembHPKey pkey ) {
        if (pkey != null) {
            if (pkey instanceof CFSecJpaSecSysGrpMembHPKey) {
                this.pkey = (CFSecJpaSecSysGrpMembHPKey)pkey;
            }
            else {
                throw new CFLibUnsupportedClassException(getClass(), "setPKey", "pkey", pkey, "CFSecJpaSecSysGrpMembHPKey");
            }
        }
    }
    ...
    public boolean equals( Object obj ) {
        ...
        else if (obj instanceof ICFSecSecSysGrpMembByLoginIdxKey) {
            ICFSecSecSysGrpMembByLoginIdxKey rhs = (ICFSecSecSysGrpMembByLoginIdxKey)obj;
			if( getRequiredLoginId() != null ) {
				if( rhs.getRequiredLoginId() != null ) {
					if( ! getRequiredLoginId().equals( rhs.getRequiredLoginId() ) ) {
						return( false );
    ...
    public String toString() {
        String ret = "<CFSecJpaSecSysGrpMembH" + getXmlAttrFragment() + "/>";
        return( ret );
    }
}
