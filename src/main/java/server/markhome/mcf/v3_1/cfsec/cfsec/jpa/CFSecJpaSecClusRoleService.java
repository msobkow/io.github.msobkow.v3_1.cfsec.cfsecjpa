...
public class CFSecJpaSecClusRoleService {
	...
	public CFSecJpaSecClusRole create(CFSecJpaSecClusRole data) {
		...
		try {
			if(data.getPKey() != null && !data.getPKey().isNull() && cfsec31SecClusRoleRepository.existsById(($implCommaIJavaOptAtomType$)data.getPKey())) {
				return( (CFSecJpaSecClusRole)(cfsec31SecClusRoleRepository.findById(($implCommaIJavaOptAtomType$)(data.getPKey())).get()));
			}
