// Description: Java 25 JPA implementation of SecTentGrpMemb history objects
...
/**
 *  CFSecJpaSecTentGrpMembH provides history objects matching the CFSecSecTentGrpMemb change history.
...
@Entity
@Table(
    name = "SecTentGrpMemb_h", schema = "CFSec31",
    indexes = {
        @Index(name = "SecTentGrpMembIdIdx_h", columnList = "auditClusterId, auditStamp, auditAction, requiredRevision, auditSessionId, SecTentGrpId, login_id", unique = true),
        @Index(name = "SecTentGrpMembTentGrpIdx_h", columnList = "SecTentGrpId", unique = false),
        @Index(name = "SecTentGrpMembLoginIdx_h", columnList = "login_id", unique = false)
    }
)
@Transactional(Transactional.TxType.SUPPORTS)
@PersistenceContext(unitName = "CFSecPU")
public class CFSecJpaSecTentGrpMembH
    implements ICFSecSecTentGrpMembH, Comparable<Object>, Serializable
{
    ...
    @EmbeddedId
    protected CFSecJpaSecTentGrpMembHPKey pkey;
	@AttributeOverrides({
		@AttributeOverride( name="bytes", column = @Column( name="CreatedByUserId", nullable=false, length=CFLibDbKeyHash256.HASH_LENGTH ) )
	})
	protected CFLibDbKeyHash256 createdByUserId = CFLibDbKeyHash256.fromHex(ICFSec$emitScopingMidfix$SecTentGrpMemb.S_INIT_CREATED_BY);
	...
	protected CFLibDbKeyHash256 updatedByUserId = CFLibDbKeyHash256.fromHex(ICFSec$emitScopingMidfix$SecTentGrpMemb.S_INIT_UPDATED_BY);

	@Column(name="UpdatedAt", nullable=false)
	protected LocalDateTime updatedAt = LocalDateTime.now();

    public CFSecJpaSecTentGrpMembH() {
            // The primary key member attributes are initialized on construction
            pkey = new CFSecJpaSecTentGrpMembHPKey();
    }
    ...
    public void setPKey( ICFSecSecTentGrpMembHPKey pkey ) {
        if (pkey != null) {
            if (pkey instanceof CFSecJpaSecTentGrpMembHPKey) {
                this.pkey = (CFSecJpaSecTentGrpMembHPKey)pkey;
            }
            else {
                throw new CFLibUnsupportedClassException(getClass(), "setPKey", "pkey", pkey, "CFSecJpaSecTentGrpMembHPKey");
            }
        }
    }
    ...
    public String toString() {
        String ret = "<CFSecJpaSecTentGrpMembH" + getXmlAttrFragment() + "/>";
        return( ret );
    }
}
