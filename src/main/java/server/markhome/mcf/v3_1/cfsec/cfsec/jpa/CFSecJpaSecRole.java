// Description: Java 25 JPA implementation of a SecRole entity definition object.
...
@Entity
@Table(
	name = "SecRole", schema = "CFSec31",
	indexes = {
		@Index(name = "SecRoleIdIdx", columnList = "SecRoleId", unique = true),
		@Index(name = "SecRoleUNameIdx", columnList = "safe_name", unique = true)
	}
)
@Transactional(Transactional.TxType.SUPPORTS)
@PersistenceContext(unitName = "CFSecPU")
public class CFSecJpaSecRole
		...
		Serializable
{
	@Id
	@AttributeOverrides({
		@AttributeOverride(name="bytes", column = @Column( name="SecRoleId", nullable=false, length=CFLibDbKeyHash256.HASH_LENGTH ) )
	})
	protected CFLibDbKeyHash256 requiredSecRoleId;
	@OneToMany(fetch=FetchType.LAZY, mappedBy="pkey.requiredContainerRole")
	protected Set<CFSecJpaSecRoleEnables> optionalChildrenEnabledByRole;
	@OneToMany(fetch=FetchType.LAZY, mappedBy="pkey.requiredContainerRole")
	protected Set<CFSecJpaSecRoleMemb> optionalChildrenMembByRole;
	...
	public List<ICFSecSecRoleEnables> getOptionalChildrenEnabledByRole() {
		List<ICFSecSecRoleEnables> retlist = new ArrayList<>(optionalChildrenEnabledByRole.size());
		for (CFSecJpaSecRoleEnables cur: optionalChildrenEnabledByRole) {
			retlist.add(cur);
		}
		return( retlist );
	}
	...
	public List<ICFSecSecRoleMemb> getOptionalChildrenMembByRole() {
		List<ICFSecSecRoleMemb> retlist = new ArrayList<>(optionalChildrenMembByRole.size());
		for (CFSecJpaSecRoleMemb cur: optionalChildrenMembByRole) {
			retlist.add(cur);
		}
		return( retlist );
	}
	...
	public String toString() {
		String ret = "<CFSecJpaSecRole" + getXmlAttrFragment() + "/>";
		return( ret );
	}
}
