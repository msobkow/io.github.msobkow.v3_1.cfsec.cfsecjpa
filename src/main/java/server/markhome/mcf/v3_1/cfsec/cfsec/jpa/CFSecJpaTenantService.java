...
public class CFSecJpaTenantService {
	...
	public CFSecJpaTenant create(CFSecJpaTenant data) {
		...
		try {
			if(data.getPKey() != null && !data.getPKey().isNull() && cfsec31TenantRepository.existsById(($implCommaIJavaOptAtomType$)data.getPKey())) {
				return( (CFSecJpaTenant)(cfsec31TenantRepository.findById(($implCommaIJavaOptAtomType$)(data.getPKey())).get()));
			}
