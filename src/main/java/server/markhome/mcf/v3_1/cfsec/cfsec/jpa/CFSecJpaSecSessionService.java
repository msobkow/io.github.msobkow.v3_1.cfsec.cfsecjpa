...
public class CFSecJpaSecSessionService {
	...
	public CFSecJpaSecSession create(CFSecJpaSecSession data) {
		...
		try {
			if(data.getPKey() != null && !data.getPKey().isNull() && cfsec31SecSessionRepository.existsById(($implCommaIJavaOptAtomType$)data.getPKey())) {
				return( (CFSecJpaSecSession)(cfsec31SecSessionRepository.findById(($implCommaIJavaOptAtomType$)(data.getPKey())).get()));
			}
