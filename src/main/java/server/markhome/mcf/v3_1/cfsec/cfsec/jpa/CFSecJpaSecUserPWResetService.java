...
public class CFSecJpaSecUserPWResetService {
	...
	public CFSecJpaSecUserPWReset create(CFSecJpaSecUserPWReset data) {
		...
		try {
			if(data.getPKey() != null && !data.getPKey().isNull() && cfsec31SecUserPWResetRepository.existsById(($implCommaIJavaOptAtomType$)data.getPKey())) {
				return( (CFSecJpaSecUserPWReset)(cfsec31SecUserPWResetRepository.findById(($implCommaIJavaOptAtomType$)(data.getPKey())).get()));
			}
