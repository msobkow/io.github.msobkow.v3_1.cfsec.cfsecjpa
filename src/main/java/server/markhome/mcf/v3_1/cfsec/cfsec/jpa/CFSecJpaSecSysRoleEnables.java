		...
		Serializable
{
	...
	protected CFLibDbKeyHash256 createdByUserId = CFLibDbKeyHash256.fromHex(ICFSec$emitScopingMidfix$SecSysRoleEnables.S_INIT_CREATED_BY);
	...
	protected CFLibDbKeyHash256 updatedByUserId = CFLibDbKeyHash256.fromHex(ICFSec$emitScopingMidfix$SecSysRoleEnables.S_INIT_UPDATED_BY);
	...
	public void setRequiredParentEnableGroup(ICFSecSecSysGrp argObj) {
		if(argObj == null) {
			...
			if (requiredParentEnableGroup != null) {
				getPKey().setRequiredEnableName(requiredParentEnableGroup.getRequiredName());
			}
			else {
			}
		}
