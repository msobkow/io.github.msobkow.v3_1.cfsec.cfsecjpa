...
public class CFSecJpaSecUserEMConfService {
	...
	public CFSecJpaSecUserEMConf create(CFSecJpaSecUserEMConf data) {
		...
		try {
			if(data.getPKey() != null && !data.getPKey().isNull() && cfsec31SecUserEMConfRepository.existsById(($implCommaIJavaOptAtomType$)data.getPKey())) {
				return( (CFSecJpaSecUserEMConf)(cfsec31SecUserEMConfRepository.findById(($implCommaIJavaOptAtomType$)(data.getPKey())).get()));
			}
