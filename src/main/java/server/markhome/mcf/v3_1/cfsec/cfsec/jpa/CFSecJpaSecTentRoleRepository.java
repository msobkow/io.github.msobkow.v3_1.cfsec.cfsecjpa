...
public interface CFSecJpaSecTentRoleRepository extends JpaRepository<CFSecJpaSecTentRole, CFLibDbKeyHash256> {
	...
	CFSecJpaSecTentRole get(@Param("secTentRoleId") CFLibDbKeyHash256 requiredSecTentRoleId);
	 ...
	 *		@return List&lt;CFSecJpaSecTentRole&gt; of the found entities, typically from the JPA cache, or an empty list if no such entities exist.
	 */
	@Query("select r from CFSecJpaSecTentRole r where r.requiredContainerRole.requiredName = :name")
	 ...
	 */
	@Query("select r from CFSecJpaSecTentRole r where r.requiredOwnerTenant.requiredId = :tenantId and r.requiredContainerRole.requiredName = :name")
	...
	@Query("select r from CFSecJpaSecTentRole r where r.requiredSecTentRoleId = :secTentRoleId")
	CFSecJpaSecTentRole lockByIdIdx(@Param("secTentRoleId") CFLibDbKeyHash256 requiredSecTentRoleId);
	...
	@Transactional
	@Modifying
	@Query("delete from CFSecJpaSecTentRole r where r.requiredContainerRole.requiredName = :name")
	...
	@Transactional
	@Modifying
	@Query("delete from CFSecJpaSecTentRole r where r.requiredOwnerTenant.requiredId = :tenantId and r.requiredContainerRole.requiredName = :name")
