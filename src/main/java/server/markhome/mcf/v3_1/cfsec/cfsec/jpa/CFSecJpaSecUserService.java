...
public class CFSecJpaSecUserService {
	...
	public CFSecJpaSecUser create(CFSecJpaSecUser data) {
		...
		try {
			if(data.getPKey() != null && !data.getPKey().isNull() && cfsec31SecUserRepository.existsById(($implCommaIJavaOptAtomType$)data.getPKey())) {
				return( (CFSecJpaSecUser)(cfsec31SecUserRepository.findById(($implCommaIJavaOptAtomType$)(data.getPKey())).get()));
			}
