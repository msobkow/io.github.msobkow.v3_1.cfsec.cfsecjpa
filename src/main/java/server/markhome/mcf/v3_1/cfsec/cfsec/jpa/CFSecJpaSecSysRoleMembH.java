// Description: Java 25 JPA implementation of SecSysRoleMemb history objects
...
/**
 *  CFSecJpaSecSysRoleMembH provides history objects matching the CFSecSecSysRoleMemb change history.
...
@Entity
@Table(
    name = "SecSysRoleMemb_h", schema = "CFSec31",
    indexes = {
        @Index(name = "SecSysRoleMembIdIdx_h", columnList = "auditClusterId, auditStamp, auditAction, requiredRevision, auditSessionId, SecSysRoleId, login_id", unique = true),
        @Index(name = "SecSysRoleMembSysRoleIdx_h", columnList = "SecSysRoleId", unique = false),
        @Index(name = "SecSysRoleMembLoginIdx_h", columnList = "login_id", unique = false)
    }
)
@Transactional(Transactional.TxType.SUPPORTS)
@PersistenceContext(unitName = "CFSecPU")
public class CFSecJpaSecSysRoleMembH
    implements ICFSecSecSysRoleMembH, Comparable<Object>, Serializable
{
	@AttributeOverrides({
    ...
    @EmbeddedId
    protected CFSecJpaSecSysRoleMembHPKey pkey;
	@AttributeOverrides({
		@AttributeOverride( name="bytes", column = @Column( name="CreatedByUserId", nullable=false, length=CFLibDbKeyHash256.HASH_LENGTH ) )
	})
	protected CFLibDbKeyHash256 createdByUserId = CFLibDbKeyHash256.fromHex(ICFSec$emitScopingMidfix$SecSysRoleMemb.S_INIT_CREATED_BY);
	...
	protected CFLibDbKeyHash256 updatedByUserId = CFLibDbKeyHash256.fromHex(ICFSec$emitScopingMidfix$SecSysRoleMemb.S_INIT_UPDATED_BY);

	@Column(name="UpdatedAt", nullable=false)
	protected LocalDateTime updatedAt = LocalDateTime.now();

    public CFSecJpaSecSysRoleMembH() {
            // The primary key member attributes are initialized on construction
            pkey = new CFSecJpaSecSysRoleMembHPKey();
    }
    ...
    @Override
    public ICFSecSecSysRoleMembHPKey getPKey() {
        return( pkey );
    }

    @Override
    public void setPKey( ICFSecSecSysRoleMembHPKey pkey ) {
        if (pkey != null) {
            if (pkey instanceof CFSecJpaSecSysRoleMembHPKey) {
                this.pkey = (CFSecJpaSecSysRoleMembHPKey)pkey;
            }
            else {
                throw new CFLibUnsupportedClassException(getClass(), "setPKey", "pkey", pkey, "CFSecJpaSecSysRoleMembHPKey");
            }
        }
    }
    ...
    public String toString() {
        String ret = "<CFSecJpaSecSysRoleMembH" + getXmlAttrFragment() + "/>";
        return( ret );
    }
}
