...
public class CFSecJpaSecUserPasswordService {
	...
	public CFSecJpaSecUserPassword create(CFSecJpaSecUserPassword data) {
		...
		try {
			if(data.getPKey() != null && !data.getPKey().isNull() && cfsec31SecUserPasswordRepository.existsById(($implCommaIJavaOptAtomType$)data.getPKey())) {
				return( (CFSecJpaSecUserPassword)(cfsec31SecUserPasswordRepository.findById(($implCommaIJavaOptAtomType$)(data.getPKey())).get()));
			}
