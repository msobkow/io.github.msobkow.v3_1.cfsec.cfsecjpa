...
public class CFSecJpaClusterService {
	...
	public CFSecJpaCluster create(CFSecJpaCluster data) {
		...
		try {
			if(data.getPKey() != null && !data.getPKey().isNull() && cfsec31ClusterRepository.existsById(($implCommaIJavaOptAtomType$)data.getPKey())) {
				return( (CFSecJpaCluster)(cfsec31ClusterRepository.findById(($implCommaIJavaOptAtomType$)(data.getPKey())).get()));
			}
