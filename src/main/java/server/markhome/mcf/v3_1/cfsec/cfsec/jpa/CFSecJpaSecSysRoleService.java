...
public class CFSecJpaSecSysRoleService {
	...
	public CFSecJpaSecSysRole create(CFSecJpaSecSysRole data) {
		...
		try {
			if(data.getPKey() != null && !data.getPKey().isNull() && cfsec31SecSysRoleRepository.existsById(($implCommaIJavaOptAtomType$)data.getPKey())) {
				return( (CFSecJpaSecSysRole)(cfsec31SecSysRoleRepository.findById(($implCommaIJavaOptAtomType$)(data.getPKey())).get()));
			}
