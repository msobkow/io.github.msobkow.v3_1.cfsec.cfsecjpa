// Description: Java 25 JPA implementation of SecTentRoleMemb history objects
...
/**
 *  CFSecJpaSecTentRoleMembH provides history objects matching the CFSecSecTentRoleMemb change history.
...
@Entity
@Table(
    name = "SecTentRoleMemb_h", schema = "CFSec31",
    indexes = {
        @Index(name = "SecTentRoleMembIdIdx_h", columnList = "auditClusterId, auditStamp, auditAction, requiredRevision, auditSessionId, SecTentRoleId, login_id", unique = true),
        @Index(name = "SecTentRoleMembTentRoleIdx_h", columnList = "SecTentRoleId", unique = false),
    ...
    implements ICFSecSecTentRoleMembH, Comparable<Object>, Serializable
{
    ...
    @EmbeddedId
    protected CFSecJpaSecTentRoleMembHPKey pkey;
	@AttributeOverrides({
		@AttributeOverride( name="bytes", column = @Column( name="CreatedByUserId", nullable=false, length=CFLibDbKeyHash256.HASH_LENGTH ) )
	})
	protected CFLibDbKeyHash256 createdByUserId = CFLibDbKeyHash256.fromHex(ICFSec$emitScopingMidfix$SecTentRoleMemb.S_INIT_CREATED_BY);
	...
	protected CFLibDbKeyHash256 updatedByUserId = CFLibDbKeyHash256.fromHex(ICFSec$emitScopingMidfix$SecTentRoleMemb.S_INIT_UPDATED_BY);

	@Column(name="UpdatedAt", nullable=false)
	protected LocalDateTime updatedAt = LocalDateTime.now();

    public CFSecJpaSecTentRoleMembH() {
            // The primary key member attributes are initialized on construction
            pkey = new CFSecJpaSecTentRoleMembHPKey();
    }
    ...
    @Override
    public ICFSecSecTentRoleMembHPKey getPKey() {
        return( pkey );
    }

    @Override
    public void setPKey( ICFSecSecTentRoleMembHPKey pkey ) {
        if (pkey != null) {
            if (pkey instanceof CFSecJpaSecTentRoleMembHPKey) {
                this.pkey = (CFSecJpaSecTentRoleMembHPKey)pkey;
            }
            else {
                throw new CFLibUnsupportedClassException(getClass(), "setPKey", "pkey", pkey, "CFSecJpaSecTentRoleMembHPKey");
            }
        }
    }
    ...
    public String toString() {
        String ret = "<CFSecJpaSecTentRoleMembH" + getXmlAttrFragment() + "/>";
        return( ret );
    }
}
