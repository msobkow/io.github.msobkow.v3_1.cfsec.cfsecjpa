...
public class CFSecJpaSysClusterService {
	...
	public CFSecJpaSysCluster create(CFSecJpaSysCluster data) {
		...
		try {
			if(data.getPKey() != null && cfsec31SysClusterRepository.existsById(($implCommaIJavaOptAtomType$)data.getPKey())) {
				return( (CFSecJpaSysCluster)(cfsec31SysClusterRepository.findById(($implCommaIJavaOptAtomType$)(data.getPKey())).get()));
			}
